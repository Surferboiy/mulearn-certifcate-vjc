document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupCardListeners(card, dragBox, elData) {
        ...
        // Text case
        card.querySelector('.text-case-select').addEventListener('change', (e) => {
            elData.text_transform = e.target.value;
            // Update preview label in drag box
            const raw = previewDataRow[elData.column] || `[${elData.column}]`;
            dragBox.textContent = applyTransform(String(raw), elData.text_transform);
        });
    ...
    function applyTransform(text, transform) {
        ...
        if (transform === 'title') return text.replace(/\w\S*/g, w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase());
    ...
    function syncBoxVisuals(dragBox, elData) {
        ...
        // Update text with correct case
        const raw = previewDataRow[elData.column] || `[${elData.column}]`;
        dragBox.textContent = applyTransform(String(raw), elData.text_transform || 'upper');
    ...
    const singlePanel = document.getElementById('single-cert-panel');
    const batchWrap  = document.getElementById('batch-generate-wrap');
    ...
    function switchMode(mode) {
        ...
        batchWrap.style.display   = mode === 'batch'  ? 'block' : 'none';

        if (mode === 'single') buildSingleInputs();
    }

    modeBatch.addEventListener('click',  () => switchMode('batch'));
    modeSingle.addEventListener('click', () => switchMode('single'));

    function buildSingleInputs() {
        singleInputs.innerHTML = '';
        elements.forEach(el => {
            const label = el.column || 'Text';
            const wrapper = document.createElement('div');
            wrapper.className = 'input-group';
            wrapper.innerHTML = `
                <label>${label}</label>
                <input type="text" class="single-val-input" data-col="${el.column}"
                       placeholder="Enter ${label}..." style="width:100%;">
            `;
            singleInputs.appendChild(wrapper);
        });
    }

    // Rebuild single inputs when elements change
    const origAddElement = btnAddElement.onclick;
    btnAddElement.addEventListener('click', () => {
        if (currentMode === 'single') setTimeout(buildSingleInputs, 50);
    });

    // --- SINGLE CERTIFICATE GENERATION ---
    document.getElementById('btn-single-generate').addEventListener('click', async () => {
        ...
        // Build manual_row from inputs
        const manualRow = {};
        singleInputs.querySelectorAll('.single-val-input').forEach(inp => {
            manualRow[inp.dataset.col] = inp.value;
        });

        try {
            const formData = new FormData();
            formData.append('template', templateInput.files[0]);
            formData.append('config', configPayload.value);
            formData.append('manual_row', JSON.stringify(manualRow));

            const response = await fetch(`${BACKEND_URL}/generate-single`, { method: 'POST', body: formData });
            ...
            const url  = window.URL.createObjectURL(blob);
            const a    = document.createElement('a');
            const firstVal = Object.values(manualRow)[0] || 'certificate';
            a.href = url;
            a.download = `${firstVal.toUpperCase()}.jpg`;
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            document.body.removeChild(a);
            ...
            // Toast
            const toast = document.getElementById('success-toast');
            document.getElementById('toast-title').textContent = '✅ Certificate Ready!';
            document.getElementById('toast-msg').textContent = `Downloaded for ${firstVal}`;
            toast.classList.add('show');
